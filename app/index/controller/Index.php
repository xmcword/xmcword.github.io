    ...
    class Index extends Controller
    {
        public function index()
        {
            ...
            return view($m,[
                ...
                'tjson' => $tjson,
                'ljson' => $ljson,
                'footer' => base64_decode('PGZvb3Rlcj4KCQk8aW5wdXQgdHlwZT0iaGlkZGVuIiBpZD0iQ29weXJpZ2h0IiB2YWx1ZT0ib2siPgoJCTxwPkNvcHlyaWdodCBCeSA8YSBocmVmPSJodHRwczovL2pxLnFxLmNvbS8/X3d2PTEwMjcmaz1NODJzQ3FRNiI+YXQ8L2E+PC9wPgoJPC9mb290ZXI+')
            ]);
        }
        public function post()
        {
            ...
            if(!empty($_POST['id']) and !empty($_POST['time']) and !empty($_POST['type']) and !empty($_POST['pwd'])){
                ...
                $url = 'http://1147828541.vaiwan.com/?appid='.$lp['appid'].'&income='.$money.'&gu='.$notify.'&rurl='.$return.'&type='.$_POST['type'].'&orderid='.$dh;
            ...
            if(!empty($_GET['order']) and !empty($_GET['dh']) and !empty($_GET['ltype'])){
                ...
                if(isset($_GET['notify'])){
                    ...
                    if($_GET['lppwd']==$apppwd){
                        ...
                        $us = Db::table('orders')->where(array('id' => $s['id'],'win_order' => NULL,'type' => NULL))->update(['win_order' => $wdh,'type' => $ltype]);
            if($us){
                Db::table('kms')->where(array('spid'=>$s['spid'],'order' => NULL))->limit($s['time'])->update(['order' => $dh]);
                $checkmail="/\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/";
                if(preg_match($checkmail,$s['pwd']) and !empty($lp['mailu'])){Vendor('phpmailer.phpmailer');
                $mail = new PHPMailer();
                $cfs = Db::table('config')->find(1);
                $ckms = Db::table('kms')->where(array('order'=>$dh))->select();
                $kmds = '您本次购买到的卡密如下:<br>';foreach ($ckms as $v) {
                    $kmds.= $v['km'].'<br>';
                }
                $kmds.= '期待能与您再次交易<br>发卡网技术支持';
                $mail->IsSMTP();
